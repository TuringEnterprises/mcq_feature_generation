...
def full_ranking_evaluation(
    ...
    dfs_eval_names: [],
    train_label_col: str = "is_start",
    eval_labels_cols: [] = None,
    pred_col: str = "preds",
    test_jobs_folds: [] = None,
    sample_weight_col: str = None,
    metrics: [] = ["pairwise_ranking_score", "raw_pairwise_ranking_score"],
    exclude_uni_label_jobs: bool = True,
    win_loss_ns: [] = [1, 3, 5, 10],
    win_loss_interview_weights: [] = [1, 0.5, 0.25, 0.2],
    ...
    for df_eval, name, label_col in zip(dfs_eval, dfs_eval_names, eval_labels_cols):
        ...
        if label_col == "is_start":
            ...
            for weight in win_loss_interview_weights:
                metric_dict[f"interview_win_loss_{weight}"] = (
                    wins.sum() - weight * losses.sum()
                )
        else:
            metric_dict["interview_win_portion"] = np.nan
            for weight in win_loss_interview_weights:
                metric_dict[f"interview_win_loss_{weight}"] = np.nan
